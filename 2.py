...
class NotionHelper:
    ...
    def update_page(self, page_id, image_url, loot):
        """ページを更新"""
        try:
            self.client.pages.update(
                ...
                properties={
                    ...
                    "Loot": {"type": "checkbox", "checkbox": loot}
                }
            )
        except Exception as e:
...
def process_item(page_id, notion_helper, discord, dropbox_helper, ocr_processor, config):
    ...
    if item and not item["properties"]["Ignore"]["checkbox"]:
        ...
        # 価格と時間のOCR抽出
        price = ocr_processor.extract_text("img/Price.png")
        time = ocr_processor.extract_text("img/Time.png")

        if price and time:
            dropbox_helper.upload_file("img/Avatar.png", "/Avatar.png")
            dropbox_helper.upload_file("img/Origin.png", f"/{item_name}.png")
            notion_helper.update_page(page_id, f"{image_url}&{time()}", loot=True)
            sleep(3)
        ...
        if price and not time:
            dropbox_helper.upload_file("img/Origin.png", f"/{item_name}.png")
            notion_helper.update_page(page_id, f"{image_url}&{time()}", loot=False) 
